...
/*哈希函数(除留余数法)*/  
int Hash(int data)  //返回hash表的对应的元素的位置
{  
    return data % m;  //m是哈希表的大小
}  
...
{  
    ...
    //发生冲突  
    while(hashTable->elem[hashAddress]!=NULLKEY)  //如果不等于初始地址，即和已经赋值的元素地址重复了，利用该方法来标记未被利用的空间
    {  
        //利用开放定址的线性探测法解决冲突  
...
{  
    ...
    //发生冲突  
    while(hashTable->elem[hashAddress]!=data)  //所求得的地址已经存放了其他的数据值
    {  
        //利用开放定址的线性探测法解决冲突  
        hashAddress=(++hashAddress)%m;  
